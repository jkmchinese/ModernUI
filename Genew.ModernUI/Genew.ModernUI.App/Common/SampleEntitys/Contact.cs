...
namespace Genew.ModernUI.App.Common.SampleDatas
{
    ...
    public class Contact : INotifyPropertyChanged, IEditableObject
    {
        ...
        [RegularExpression(@"^\(\d\d\d\) \d\d\d\-\d\d\d\d$",
            ErrorMessage =
                "Not a valid phone number.  Please enter a phone number that matches the format (###) ###-####")]
        public string Phone
        {
            get { return phone; }
            set
        ...
        /// </summary>
        [Display(Name = "Secondary Street Address",
            Description = "Additional street address information, such as apartment number or P.O. Box",
            GroupName = "Address")]
        ...
        /// </summary>
        [RegularExpression(
            @"^([a-zA-Z0-9\!\%\$\%\*\/\?\|\^\{\}\`\~\&\'\+\-\=_]\.?)*[a-zA-Z0-9\!\%\$\%\*\/\?\|\^\{\}\`\~\&\'\+\-\=_]@((([a-zA-Z0-9\!\%\$\%\*\/\?\|\^\{\}\`\~\&\'\+\-\=_]\.?)*[a-zA-Z0-9\!\%\$\%\*\/\?\|\^\{\}\`\~\&\'\+\-\=_])|(\[\d+\.\d+\.\d+\.\d+\]))$",
