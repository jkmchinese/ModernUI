...
namespace ModernUI.ExtendedToolkit
{
    ...
    public class MaskedTextBox : ValueRangeTextBox
    {
        ...
        {
            TextBox.TextProperty.OverrideMetadata(typeof(MaskedTextBox),
        ...
        public MaskedTextBox()
        {
            ...
            System.Windows.DragDrop.AddPreviewQueryContinueDragHandler(this, PreviewQueryContinueDragCallback);
            AllowDrop = false;
        }
        ...
        {
            string preInitializedText = Text;

            string mask = Mask;

            if (mask == string.Empty)
            {
                m_maskedTextProvider = CreateMaskedTextProvider(MaskedTextBox.NullMaskString);
                m_maskIsNull = true;
            }
            else
            {
        ...
        public static readonly DependencyProperty IsMaskCompletedProperty =
            MaskedTextBox.IsMaskCompletedPropertyKey.DependencyProperty;


        public bool IsMaskCompleted
        {
            get { return (bool)GetValue(MaskedTextBox.IsMaskCompletedProperty); }
        }

        private void SetIsMaskCompleted(bool value)
        {
            SetValue(MaskedTextBox.IsMaskCompletedPropertyKey, value);
        }
        ...
        public static readonly DependencyProperty IsMaskFullProperty =
            MaskedTextBox.IsMaskFullPropertyKey.DependencyProperty;

        public bool IsMaskFull
        {
            get { return (bool)GetValue(MaskedTextBox.IsMaskFullProperty); }
        }

        private void SetIsMaskFull(bool value)
        {
            SetValue(MaskedTextBox.IsMaskFullPropertyKey, value);
        }
        ...
        public string Mask
        {
            get { return (string)GetValue(MaskedTextBox.MaskProperty); }
            set { SetValue(MaskedTextBox.MaskProperty, value); }
        }
        ...
        {
            get { return (char)GetValue(MaskedTextBox.PromptCharProperty); }
            set { SetValue(MaskedTextBox.PromptCharProperty, value); }
        }
        ...
        private string ValidateText(string text)
        {
            ...
            else
            {
                ...
                if (CanReplace(provider, text, 0, m_maskedTextProvider.Length, RejectInputOnFirstFailure, out caretIndex))
                {
                    coercedText = GetFormattedString(provider);
                }
                else
        ...
        private void CanExecuteDeletePreviousWord(object sender, CanExecuteRoutedEventArgs e)
        {
            ...
            bool canDeletePreviousWord = (!IsReadOnly) && (EditingCommands.SelectLeftByWord.CanExecute(null, this));

            if (canDeletePreviousWord)
            {
                int cachedSelectionStart = SelectionStart;
                int cachedSelectionLength = SelectionLength;
        ...
        private void CanExecuteDeleteNextWord(object sender, CanExecuteRoutedEventArgs e)
        {
            ...
            bool canDeleteNextWord = (!IsReadOnly) && (EditingCommands.SelectRightByWord.CanExecute(null, this));
        ...
        private void CanExecuteCut(object sender, CanExecuteRoutedEventArgs e)
        {
            ...
            bool canCut = (!IsReadOnly) && (SelectionLength > 0);

            if (canCut)
            {
                int endPosition = (SelectionLength > 0) ? ((SelectionStart + SelectionLength) - 1) : SelectionStart;
        ...
        protected override string QueryTextFromValueCore(object value)
        {
            if ((m_valueToStringMethodInfo != null) && (value != null))
            {
                try
                {
                    string text =
                        (string)
                            m_valueToStringMethodInfo.Invoke(value,
                                new object[] { m_formatSpecifier, GetActiveFormatProvider() });
                    return text;
                }
                catch
                {
        ...
        private void ProcessTextInput(string text)
        {
            if (text.Length == 1)
            {
                string textOutput = MaskedTextOutput;
        ...
        protected override void ValidateValue(object value)
        {
            ...
            if (!m_maskIsNull)
            {
                string representation = GetTextFromValue(value);
        ...
        internal override string GetCurrentText()
        {
            ...
            string displayText = GetFormattedString(m_maskedTextProvider);
        ...
        internal override void RefreshConversionHelpers()
        {
            ...
            char[] maskChars = GetMaskCharacters();
            ...
            NumberFormatInfo numberFormatInfo =
                activeFormatProvider.GetFormat(typeof(NumberFormatInfo)) as NumberFormatInfo;
            ...
            out int caretPosition)
        {
            if (ShouldQueryAutoCompleteMask(provider.Clone() as MaskedTextProvider, ch, startPosition))
            {
                ...
                if ((!e.Cancel) && (e.AutoCompleteStartPosition > -1))
                {
                    ...
                    {
                        if (
                            !PlaceCharCore(provider, e.AutoCompleteText[i], e.AutoCompleteStartPosition + i, 0, true,
                                out caretPosition))
                            return false;
                    }
            ...
            if (provider.IsEditPosition(startPosition))
            {
                ...
                if (nextSeparatorIndex != -1)
                {
                    if (provider[nextSeparatorIndex].Equals(ch))
                    {
                        ...
                        if (
                            provider.FindUnassignedEditPositionInRange(previousSeparatorIndex, nextSeparatorIndex, true) !=
                            -1)
                        {
                            return true;
                        }
        ...
        internal void Replace(string text, int startPosition, int selectionLength)
        {
            ...
            {
                System.Diagnostics.Debug.WriteLine("Replace caret index to: " + tentativeCaretIndex);

                bool mustRefreshText = MaskedTextOutput != provider.ToString();
                m_maskedTextProvider = provider;
        ...
        private void Delete(int startPosition, int selectionLength, bool deleteForward)
        {
            ...
            string oldTextOutput = MaskedTextOutput;

            bool success = m_maskedTextProvider.RemoveAt(startPosition, endPosition, out tentativeCaretPosition,
                out hint);
        ...
        private string GetFormattedString(MaskedTextProvider provider)
        {
            ...
            bool includePrompt = (IsReadOnly) ? false : (!HidePromptOnLeave || IsFocused);
        ...
        private string GetSelectedText()
        {
            ...
            bool includeLiterals = (ClipboardMaskFormat & MaskFormat.IncludeLiterals) !=
